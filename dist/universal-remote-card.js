...
`;let _t=class extends at{static get styles(){return[ft]}setConfig(t){this._config=t}_valueChanged(t){if(!this._config||!this.hass)return;const e=t.target,i=e.value,r=e.configValue;r&&(this._config={...this._config,[r]:i},this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:this._config}})))}render(){if(!this.hass||!this._config)return I``;const t=Object.keys(this.hass.states).filter(t=>t.startsWith("media_player.")),e=Object.keys(this.hass.states).filter(t=>t.startsWith("remote."));return I`
      <div class="editor">
    ...
    `:I`<div class="spacer"></div>`}_renderDpad(){return I`
    ...
    `}_renderStatusBar(){const{media_player_id:t}=this._config;if(!t||!this.hass)return W;const e=this.hass.states[t];if(!e)return W;const i=e.state,r=e.attributes.media_title||"";return I`
    ...
    `}render(){if(!this._config)return I``;const{title:t,platform:e="Fire TV",rows:i,color_scheme:r="dark"}=this._config;return I`
      <ha-card>
